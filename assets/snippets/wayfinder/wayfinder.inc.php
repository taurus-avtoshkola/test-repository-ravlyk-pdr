...
class Wayfinder {
    ...
    function getData() {
        ...
        if (!empty($ids)) {
            ...
            // languages injection begin
            if ($modx->config['lang_enable'])
                $fields = "sc.id, sc.menutitle_".$modx->config['lang']."  as 'menutitle', sc.pagetitle_".$modx->config['lang']."  as 'pagetitle', sc.introtext_".$modx->config['lang']."  as 'introtext', sc.menuindex, sc.published, sc.hidemenu, sc.parent, sc.isfolder, sc.description_".$modx->config['lang']."  as 'description', sc.alias, sc.longtitle_".$modx->config['lang']."  as 'longtitle', sc.type,if(sc.type='reference',sc.content_".$modx->config['lang'].",'') as content, sc.template, sc.link_attributes_".$modx->config['lang']." as 'link_attributes'";
            else
                $fields = "sc.id, sc.menutitle, sc.pagetitle, sc.introtext, sc.menuindex, sc.published, sc.hidemenu, sc.parent, sc.isfolder, sc.description, sc.alias, sc.longtitle, sc.type,if(sc.type='reference',sc.content,'') as content, sc.template, sc.link_attributes";
        // languges injection end
