...
<table class="table table-condensed table-striped table-bordered table-hover">
	<thead>
		<tr>
			...
			<th>Вебінар <nobr><span class="ord" data-order="ORDER BY w.webi_name DESC">&uarr;</span><span class="ord" data-order="ORDER BY w.webi_name ASC">&darr;</span></nobr></th>
	...
	<tbody>
		...
		?>
			<tr>
				...
				<td>
					<a data-confirm="Ви впевнені, що хочете видалити?" href="<?=$url?>b=webi&c=delete&i=<?=$pro['id']?>" class="btn btn-mini btn-danger"><span class="glyphicon glyphicon-remove-sign"></span> <?=$_lang["shop_delete"]?> </a>
				</td>
			</tr>
		<? endwhile;endif; ?>		
	</tbody>
...
$(document).ready(function(){
    $(document).on('click', '.ord', function(){
    	window.location.replace("<?=$url?>b=webi&orderby="+$(this).data('order'));
    });
});
</script>