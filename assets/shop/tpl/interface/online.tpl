...
<table class="table table-condensed table-striped table-bordered table-hover">
	<thead>
		<tr>
			...
			<th width="50px">Код оплати <nobr><span class="ord" data-order="ORDER BY o.hash DESC">&uarr;</span><span class="ord" data-order="ORDER BY o.hash ASC">&darr;</span></nobr></th>
			...
			<th>Промокод <nobr><span class="ord" data-order="ORDER BY o.promocode DESC">&uarr;</span><span class="ord" data-order="ORDER BY o.promocode ASC">&darr;</span></nobr></th>
			<th>Сума <nobr><span class="ord" data-order="ORDER BY o.pay_sum DESC">&uarr;</span><span class="ord" data-order="ORDER BY o.pay_sum ASC">&darr;</span></nobr></th>
			<th>Статус оплати <nobr><span class="ord" data-order="ORDER BY o.status_pay DESC">&uarr;</span><span class="ord" data-order="ORDER BY o.status_pay ASC">&darr;</span></nobr></th>
	...
	<tbody>
		...
		?>
			<tr>
				...
				<td><?=$pro['promocode']?></td>
				<td><?=$pro['pay_sum']?></td>
				<td><?php if($pro['status_pay'] == '1'){echo '<span class="label label-success">Сплачено</span>';}else{echo '<span class="label label-danger">Не сплачено</span>';}?></td>
				...
				<td>
					<a data-confirm="Ви впевнені, що хочете видалити?" href="<?=$url?>b=online&c=delete&i=<?=$pro['id']?>" class="btn btn-mini btn-danger"><span class="glyphicon glyphicon-remove-sign"></span> <?=$_lang["shop_delete"]?> </a>
				</td>
			</tr>
		<? endwhile;endif; ?>		
	</tbody>
...
$(document).ready(function(){
    $(document).on('click', '.ord', function(){
    	window.location.replace("<?=$url?>b=online&orderby="+$(this).data('order'));
    });
});
</script>