	...
	<tbody>
		...
		?>
			<tr>
				...
				<td>
					<a data-confirm="Ви впевнені, що хочете видалити?" href="<?=$url?>b=webi_in&c=delete&i=<?=$pro['id']?>" class="btn btn-mini btn-danger"><span class="glyphicon glyphicon-remove-sign"></span> <?=$_lang["shop_delete"]?> </a>
				</td>
			</tr>
		<? endwhile;endif; ?>		
	</tbody>
...
$(document).ready(function(){
    $(document).on('click', '.ord', function(){
    	window.location.replace("<?=$url?>b=webi_in&orderby="+$(this).data('order'));
    });
});
</script>