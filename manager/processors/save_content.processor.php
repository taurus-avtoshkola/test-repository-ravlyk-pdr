...
if ($modx->config['lang_enable']) {
    $languages       = explode(",", $modx->config["lang_list"]);
        foreach($languages as $lng){
            ...
            $introtext[$lng] = $modx->getDatabase()->escape($_POST['introtext_'.$lng]);
            $content[$lng] = $modx->getDatabase()->escape($_POST['ta_'.$lng]);
            $pagetitle[$lng] = $modx->getDatabase()->escape($_POST['pagetitle_'.$lng]);
            $description[$lng] = $modx->getDatabase()->escape($_POST['description_'.$lng]);
            $link_attributes[$lng] = $modx->getDatabase()->escape($_POST['link_attributes_'.$lng]);
            $longtitle[$lng] = $modx->getDatabase()->escape($_POST['longtitle_'.$lng]);
            $menutitle[$lng] = $modx->getDatabase()->escape($_POST['menutitle_'.$lng]);
...
$no_esc_pagetitle = strtolower(trim(($modx->config['lang_enable'] ? $pagetitle[$modx->config['lang_default']] : $pagetitle)));
...
if ($modx->getConfig('friendly_urls')) {
    ...
    if (!$alias && $modx->getConfig('automatic_alias')) {
        ...
        $alias = strtolower($modx->stripAlias(trim(($modx->config['lang_enable'] ? $pagetitle[$modx->config['lang_default']] : $pagetitle))));
...
switch ($actionToTake) {
        ...
        if ($modx->config["lang_enable"]) {
            $dbInsert_t = array();
            foreach($languages as $lng) {
                $dbInsert_t["introtext_".$lng]       = $introtext[$lng];
                $dbInsert_t["content_".$lng]         = $content[$lng];
                $dbInsert_t["pagetitle_".$lng]       = $pagetitle[$lng];
                $dbInsert_t["longtitle_".$lng]       = $longtitle[$lng];
                $dbInsert_t["description_".$lng]     = $description[$lng];
                $dbInsert_t["link_attributes_".$lng] = $link_attributes[$lng];
                $dbInsert_t["menutitle_".$lng]       = $menutitle[$lng];
            }
            $dbInsert_t["pagetitle"]       = $pagetitle[$modx->config['lang_default']];
        ...
        $dbInsert = array_merge ($dbInsert_t, $dbInsert_s);
        ...
        case 'edit' :
            ...
            if ($modx->config["lang_enable"]) {
                $update_s = '';
                foreach($languages as $lng) {
                    $update_s .= "introtext_".$lng."='".$modx->getDatabase()->escape($_POST['introtext_'.$lng])."', 
                        content_".$lng."='".$modx->getDatabase()->escape($_POST['ta_'.$lng])."', 
                        pagetitle_".$lng."='".$modx->getDatabase()->escape($_POST['pagetitle_'.$lng])."', 
                        longtitle_".$lng."='".$modx->getDatabase()->escape($_POST['longtitle_'.$lng])."', 
                        description_".$lng."='".$modx->getDatabase()->escape($_POST['description_'.$lng])."', 
                        link_attributes_".$lng."='".$modx->getDatabase()->escape($_POST['link_attributes_'.$lng])."', 
                        menutitle_".$lng."='".$modx->getDatabase()->escape($_POST['menutitle_'.$lng])."', ";
                }
                $update_s .= " pagetitle ='".$modx->getDatabase()->escape($_POST['pagetitle_'.$modx->config['lang_default']])."',  ";
            } else {

                $update_s = "introtext='{$introtext}', "
                    . "content='{$content}', "
                    . "pagetitle='{$pagetitle}', "
                    . "longtitle='{$longtitle}', "
                    . "description='{$description}', "
