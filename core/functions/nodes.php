...
if(!function_exists('makeHTML')) {
    ...
    function makeHTML($indent, $parent, $expandAll, $hereid = '')
    {
        ...
        $result = $modx->getDatabase()->select(
            "DISTINCT sc.id, pagetitle,pagetitle_ru,pagetitle_en,pagetitle_ua, longtitle, menutitle, parent, isfolder, published, pub_date, unpub_date, richtext, searchable, cacheable, deleted, type, template, templatename, menuindex, donthit, hidemenu, alias, contentType, privateweb, privatemgr,
        MAX(IF(1={$mgrRole} OR sc.privatemgr=0 {$docgrp_cond}, 1, 0)) AS hasAccess, GROUP_CONCAT(document_group SEPARATOR ',') AS roles"
            ...
            , sprintf('(parent=%d) %s GROUP BY sc.id, pagetitle,pagetitle_ru,pagetitle_en,pagetitle_ua, longtitle, menutitle, parent, isfolder, published, pub_date, unpub_date, richtext, searchable, cacheable, deleted, type, template, templatename, menuindex, donthit, hidemenu, alias, contentType, privateweb, privatemgr'
                , $parent
                , $access
            )
            , $orderby
        ...
        while ($row = $modx->getDatabase()->getRow($result)) {
            ...
            $data = array(
                ...
                'pagetitle_ru'     => $row['pagetitle_ru'],
                'pagetitle_ua'     => $row['pagetitle_ua'],
                'pagetitle_en'     => $row['pagetitle_en'],
...
if(!function_exists('getNodeTitle')) {
    ...
    function getNodeTitle($nodeNameSource, $row)
    {
        ...
        switch ($nodeNameSource) {
            ...
            break;
            case 'pagetitle_ru':
                $nodetitle = $row['pagetitle_ru'];
            break;
            case 'pagetitle_ua':
                $nodetitle = $row['pagetitle_ua'];
            break;
            case 'pagetitle_en':
                $nodetitle = $row['pagetitle_en'];
            break;
            case 'longtitle':
