...
class UrlProcessor
{
    ...
    public function makeUrl(int $id, string $alias = '', string $args = '', string $scheme = '') : string
    {
        ...
        if ($this->core->config['lang_enable']){
              /*
            if($this->core->config['lang'] != ''){
                $host .= $this->core->config['lang']."/";
            }else{
                $host .= $this->core->config['lang_default']."/";
            }
            */
            if($this->core->config['lang'] != 'ua'){
                $host .= $this->core->config['lang']."/";
            }
        }
    ...
    public function strictURI(string $query, int $id) :? string
    {
        ...
        if($_GET['p'] != '' AND $_GET['p'] != '1' AND $_GET['p'] != '0'){
            $strictURL .= 'page-'.$_GET['p'].'/';
        }
        ...
        if ($id === (int)$this->core->getConfig('site_start')) {
            ...
            if (preg_match('#/\?q\=' . $strictURL . '#i', $_SERVER['REQUEST_URI']) || ($url_path != $strictURL && $id !== (int)$this->core->getConfig('error_page')) ) {
                if (!empty($qstring)) {
