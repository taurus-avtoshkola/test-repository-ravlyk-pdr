...
class Core extends AbstractLaravel implements Interfaces\CoreInterface
{
    ...
    public function initialize()
    {
        ...
        $this->lang = UrlProcessor::cleanQueryString(is_cli() ? '' : get_by_key($_GET, 'lang', ''));
    }
    ...
    public function getDocumentIdentifier($method)
    {
        ...
        if ($method === 'alias') {
            $n_id = $this->getDatabase()->escape($_REQUEST['q']);
            ...
            if( preg_match("/(^page-)+[0-9]{1,}\$/",end($parts)) ){
              ...
              $_GET['p'] = $page_num;
              $n_id = str_replace('page-'.$page_num.'/', '', $n_id);
            }else{
              $_GET['p'] = '1';
            }
            ...
            foreach ($parts as $part) {
                ...
                $filter_slugs = explode(',',$this->getConfig('filter_slugs'));

                if(in_array($exp_part[0], $filter_slugs)){
                    switch ($exp_part[0]) {
                        case 'type':
                            $filter_name = array_shift($exp_part);
                            $filter_val = implode('-',$exp_part);
                            $_GET['type'] = $filter_val;
                            $n_id = str_replace($part.'/', '', $n_id);
                        break;
                        case 'transmission':
                            $filter_name = array_shift($exp_part);
                            $filter_val = implode('-',$exp_part);
                            $_GET['transmission'] = $filter_val;
                            $n_id = str_replace($part.'/', '', $n_id);
                        break;
                        case 'duration':
                            $filter_name = array_shift($exp_part);
                            $filter_val = implode('-',$exp_part);
                            $_GET['duration'] = $filter_val;
                            $n_id = str_replace($part.'/', '', $n_id);
                        break;
                        case 'verify':
                            $filter_name = array_shift($exp_part);
                            $filter_val = implode('-',$exp_part);
                            $_GET['verify'] = $filter_val;
                            $n_id = str_replace($part.'/', '', $n_id);
                        break;
                        case 'sort':
                            $filter_name = array_shift($exp_part);
                            $filter_val = implode('-',$exp_part);
                            $_GET['sort'] = $filter_val;
                            $n_id = str_replace($part.'/', '', $n_id);
                        break;
                        case 'district':
                            $filter_name = array_shift($exp_part);
                            $filter_val = implode('-',$exp_part);
                            $_GET['district'] = $filter_val;
                            $n_id = str_replace($part.'/', '', $n_id);
                        break;
                        case "city":
                            $filter_name = array_shift($exp_part);
                            $filter_val = implode('-',$exp_part);
                            $_GET['city'] = $filter_val;
                            $n_id = str_replace($part.'/', '', $n_id);
                        break;
                    }
                }
    ...
    {   
        ...
        if($this->config['lang_enable'] == '1'){
            $n_url = (string)$this->lang.'/'.(string)$this->q;
        }else{
            $n_url = (string)$this->q;
        }


        $n_scheme = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on') ? 'https' : 'http';
        $n_http_host = $_SERVER['HTTP_HOST'];
        $n_url_query_string = explode('?', $_SERVER['REQUEST_URI']);
        $n_qstring = !empty($n_url_query_string[1]) ? preg_replace("#(^|&)(q|id)=[^&]+#", '', $n_url_query_string[1]) : '';
        
        
        if (!empty($n_qstring)) {
            $n_requestedURL = "{$n_scheme}://{$n_http_host}" . '/' . $n_url.'?'.$n_qstring;
        }else{
            $n_requestedURL = "{$n_scheme}://{$n_http_host}" . '/' . $n_url;
        }

        $url = UrlProcessor::strictURI((string)$this->q, (int)$this->documentIdentifier);

        if ($url !== null) {
            if($n_requestedURL != $url){
    ...
    public function sendmail($params = array(), $msg = '', $files = array())
    {
        ...
        $mail->setFrom(
            isset($p['from']) ? $p['from'] : $this->getConfig('smtp_username'),
            isset($p['fromname']) ? $p['fromname'] : $this->getConfig('site_name')
        );
        
        //test
        $mail->SMTPDebug  = 0;
        ///$mail->SetFrom($this->config['smtp_username'], $this->config['site_name']);
