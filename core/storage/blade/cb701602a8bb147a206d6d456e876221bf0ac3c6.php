...
<head>
    <title><?php echo e($modx->getConfig('site_name')); ?> - (EVO CMS Manager)</title>
    ...
    <script src="<?php echo e(ManagerTheme::getThemeUrl()); ?>js/modx.min.js?v=<?= EVO_INSTALL_TIME ?>"></script>
    <?php if ($modx->getConfig('show_picker')) { ?>
...
<div id="frameset">
    ...
    <div id="tree"><?php echo $__env->make('manager::frame.tree', array_except(get_defined_vars(), array('__data', '__path')))->render(); ?></div>
    <div id="main">
    ...
    <script type="text/javascript">
        ...
        if (document.getElementById('treeMenu')) {
            ...
            document.getElementById('treeMenu_openimages').onclick = function(e) {
                e.preventDefault();
                if (modx.config.global_tabs && !e.shiftKey) {
                    modx.tabs({url: '<?= MODX_MANAGER_URL ?>media/browser/<?php echo e($modx->getConfig('which_browser')); ?>/browse.php?filemanager=media/browser/<?php echo e($modx->getConfig('which_browser')); ?>/browse.php&type=images', title: '<?= $_lang["images_management"] ?>'});
                } else {
                    ...
                    modx.openWindow({
                        url: '<?= MODX_MANAGER_URL ?>media/browser/<?php echo e($modx->getConfig('which_browser')); ?>/browse.php?&type=images',
                        title: randomNum
                    });
                }
            };
            ...
            document.getElementById('treeMenu_openfiles').onclick = function(e) {
                e.preventDefault();
                if (modx.config.global_tabs && !e.shiftKey) {
                    modx.tabs({url: '<?= MODX_MANAGER_URL ?>media/browser/<?php echo e($modx->getConfig('which_browser')); ?>/browse.php?filemanager=media/browser/<?php echo e($modx->getConfig('which_browser')); ?>/browse.php&type=files', title: '<?= $_lang["files_files"] ?>'});
                } else {
                    var randomNum = '<?= $_lang["files_files"] ?>';
                    if (e.shiftKey) {
                        randomNum += ' #' + Math.floor((Math.random() * 999999) + 1);
                    ...
                    modx.openWindow({
                        url: '<?= MODX_MANAGER_URL ?>media/browser/<?php echo e($modx->getConfig('which_browser')); ?>/browse.php?&type=files',
                        title: randomNum
                    });
                }
            };
